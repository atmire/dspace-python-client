...
- **`examples/access_load_test/`** - Simulate access/read load (humans via real headless browsers, good bots, bad bots, or a mix) and find where a DSpace instance degrades. Real browser fan-out for humans, robots-obeying and robots-ignoring crawlers, unique-per-run search queries, disk-safe streamed downloads, and human/extended/JSON reports. **Only run against instances you control**; `*.dspace.org` is hard-blocked. Needs the `loadtest` extra (Playwright) for the human scenarios. See **`examples/access_load_test/README.md`**.
